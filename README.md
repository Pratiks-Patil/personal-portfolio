# 🌐 Pratik Patil | Personal Portfolio

<div align="center">

### 🚀 Building Intelligent Solutions with Cloud, DevOps & Innovation

[![Portfolio](https://img.shields.io/badge/Portfolio-Live-00CFFF?style=for-the-badge)]((https://pratikspatil.netlify.app/))
[![GitHub](https://img.shields.io/badge/GitHub-Pratiks--Patil-181717?style=for-the-badge&logo=github)](https://github.com/Pratiks-Patil)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Pratik%20Patil-0A66C2?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/pratikpatil03)

</div>

---

## 👋 About

Welcome to my personal portfolio!
Live Demo:https://pratikspatil.netlify.app/
I'm **Pratik Patil**, a Computer Engineering graduate currently pursuing **M.Tech in Computer Engineering at Veermata Jijabai Technological Institute (VJTI), Mumbai**.

This portfolio showcases my journey, technical skills, projects, and passion for building scalable software solutions using **Cloud Computing, DevOps, Python, AI, and modern web technologies**.

---

## ✨ Features

- 🎨 Premium dark-themed responsive UI
- 👤 Interactive avatar-based design
- 🛤️ Animated academic & professional journey timeline
- 🛠️ Modern Tech Stack showcase
- 💼 Featured project section
- 📱 Mobile-friendly layout
- 🌙 Smooth animations and micro-interactions
- 📬 Contact section with integrated social links

---

## 🛠️ Tech Stack

### 💻 Languages
- Python
- C
- C++
- HTML5
- CSS3

### ☁️ Cloud & DevOps
- AWS
- Microsoft Azure
- Docker
- Kubernetes
- Jenkins
- Linux

### 🌐 Frameworks & Technologies
- Flask
- Django
- REST APIs

### 🗄️ Databases
- MySQL
- Oracle Database
- MongoDB
- SQLite

### 🤖 AI & Data Science
- Data Analysis
- Big Data
- Natural Language Processing (NLP)
- Computer Vision
- Generative AI

### 🔧 Developer Tools
- Git
- GitHub
- VS Code
- Jupyter Notebook
- Google Colab

---

## 🎓 Journey

- 🏫 **2018** – SSC, St. Francis High School, Nashik
- 📚 **2020** – HSC, Matoshri Junior College
- 🎓 **2020–2024** – B.Tech in Computer Engineering, MIT Academy of Engineering, Pune
- 💼 **2024** – IT Internship at **Hindustan Aeronautics Limited (HAL)** *(Maharatna PSU, Ministry of Defence, Government of India)*
- 🏛️ **2025–2027** – M.Tech in Computer Engineering, **VJTI Mumbai**
- 🚀 **Future** – Continuously learning, building, and exploring new technologies

---

## 🚀 Running Locally

```bash
git clone https://github.com/Pratiks-Patil/personal-portfolio.git

cd personal-portfolio
```

Open `index.html` in your browser or use a local development server.

---

## 📂 Project Structure

```text
personal-portfolio/
│── assets/
│   ├── images/
│   ├── logos/
│── index.html
│── style.css
│── script.js
│── README.md
```

---

## 📫 Connect With Me

- 💼 LinkedIn: https://www.linkedin.com/in/pratikpatil03
- 💻 GitHub: https://github.com/Pratiks-Patil
- 📧 Email: pratikspatil372@gmail.com

---

<div align="center">

### ⭐ If you like this project, consider giving it a star!

**Designed and developed with passion by Pratik Patil.**

</div>
